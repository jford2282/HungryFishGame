...
public class CoralPillar extends Entity {
	...
	public final static String BODY_FOOD = "fishFood";
	...
	public CoralPillar(float x, float y, TextureRegion reg, TextureRegion regUp,TextureRegion fishFoodReg, VertexBufferObjectManager vbom, PhysicsWorld physics) {
		super(x, y);
		//float shift = 80;
		...
		fishFood = new Sprite(0, shift-115 , 70, 70, fishFoodReg, vbom);
		fishFood.setAnchorCenterY(0);

		attachChild(fishFood);

		fishFoodBody = PhysicsFactory.createBoxBody(physics,fishFood, BodyType.StaticBody, WALL_FIXTURE);
		fishFoodBody.setUserData(BODY_FOOD);
	...
	public boolean isFoodVisible(){
		return fishFood.isVisible();
	}
	public void setFoodVisible(){
		fishFood.setVisible(true);
	}
	public void detachFood(){
		fishFood.detachSelf();
		fishFoodBody.setActive(false);
	}
	...
	public Body getFishFoodBody() {
		if(!fishFood.hasParent())
			attachChild(fishFood);
//		Vector2 v = fishFoodBody.getLinearVelocity();
//		v.x=SPEED_X;
//		v.y=SPEED_Y;
//		fishFoodBody.setLinearVelocity(v);
//		fishFoodBody.setActive(true);
		return fishFoodBody;
	}

	public void setFishFoodBody(Body fishFoodBody) {
		this.fishFoodBody = fishFoodBody;
	}
